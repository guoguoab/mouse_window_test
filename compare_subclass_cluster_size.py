#!/usr/bin/env python3
"""Compare true and random cluster sizes for each subclass and draw an SVG plot.

Each row in a ``*_merged_regions_table_cell_id.txt`` file represents one cluster;
``total_cell_num`` is its size.  True cluster sizes are compared with the mean
cluster size from each random replicate (normally sample_1 ... sample_500).
Random files are parsed in parallel because that is the expensive input step.
"""
...
def load_random_parallel(
    ...
    return {s: dict(v) for s, v in by_sample.items()}, len(files)


def empirical_pvalues(true_value: float, random_values: Sequence[float]) -> tuple[float, float, float]:
    n = len(random_values)
    if n == 0:
...
def compute_results(true: dict[str, list[float]], random: dict[str, dict[str, list[float]]]):
    ...
    for subclass in sorted(true):
        ...
        for sample in samples:
            ...
            if not sizes:
                continue
            sample_mean = mean(sizes)
            replicate_means.append(sample_mean)
            detail.append({
                "subclass": subclass, "sample": sample,
                "random_cluster_n": len(sizes), "random_mean_cluster_size": sample_mean,
                "true_mean_cluster_size": true_mean,
                "difference_true_minus_random": true_mean - sample_mean,
            })
        if not replicate_means:
            continue
        p_gt, p_lt, p_two = empirical_pvalues(true_mean, replicate_means)
        random_mean = mean(replicate_means)
        summary.append({
            "subclass": subclass, "true_cluster_n": len(true_sizes),
            "true_mean_cluster_size": true_mean, "true_median_cluster_size": median(true_sizes),
            "random_sample_n": len(replicate_means), "random_mean_cluster_size": random_mean,
            "random_median_cluster_size": median(replicate_means),
            "random_q1": quantile(replicate_means, 0.25), "random_q3": quantile(replicate_means, 0.75),
            "random_min": min(replicate_means), "random_max": max(replicate_means),
            "difference_true_minus_random_mean": true_mean - random_mean,
            "fold_change_true_over_random": true_mean / random_mean if random_mean else math.nan,
            "p_one_sided_true_gt": p_gt, "p_one_sided_true_lt": p_lt, "p_two_sided": p_two,
        })
...
def draw_svg(rows: list[dict[str, object]], path: Path) -> None:
    ...
    width = max(1000, 105 + 70 * len(rows)); height = 700
    left, right, top, bottom = 85, 35, 70, 230
    chart_h = height - top - bottom; chart_w = width - left - right
    maximum = max(max(float(r["random_max"]), float(r["true_mean_cluster_size"])) for r in rows) * 1.12 or 1
    y = lambda value: top + chart_h * (1 - float(value) / maximum)
    out = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}">',
           '<rect width="100%" height="100%" fill="white"/>',
           f'<text x="{width/2}" y="35" text-anchor="middle" font-size="24" font-family="sans-serif" font-weight="bold">True vs random mean cluster size by subclass</text>']
    for tick in range(6):
        value = maximum * tick / 5; py = y(value)
        out += [f'<line x1="{left}" y1="{py:.1f}" x2="{width-right}" y2="{py:.1f}" stroke="#dddddd"/>',
                f'<text x="{left-8}" y="{py+5:.1f}" text-anchor="end" font-size="12" font-family="sans-serif">{value:.0f}</text>']
    step = chart_w / len(rows)
    for index, row in enumerate(rows):
        x = left + step * (index + 0.5); box_w = min(30, step * 0.55)
        low, q1, med, q3, high = (float(row[k]) for k in ("random_min", "random_q1", "random_median_cluster_size", "random_q3", "random_max"))
        out += [f'<line x1="{x:.1f}" y1="{y(low):.1f}" x2="{x:.1f}" y2="{y(high):.1f}" stroke="#d97820" stroke-width="2"/>',
        ...
        out += [f'<polygon points="{x:.1f},{true_y-6:.1f} {x+6:.1f},{true_y:.1f} {x:.1f},{true_y+6:.1f} {x-6:.1f},{true_y:.1f}" fill="#2878b5"/>',
    ...
    out += [f'<text transform="translate(20,{top+chart_h/2}) rotate(-90)" text-anchor="middle" font-size="15" font-family="sans-serif">Mean total_cell_num per cluster</text>',
            f'<rect x="{width-300}" y="50" width="14" height="14" fill="#f5ad63" stroke="#b85c00"/><text x="{width-280}" y="62" font-size="12" font-family="sans-serif">random samples</text>',
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="按 subclass 比较真实与随机 cluster size，并绘制 SVG")
    ...
    parser.add_argument("--workers", type=int, default=min(20, os.cpu_count() or 1), help="并行读取随机文件的进程数")
    parser.add_argument("--out-summary", type=Path, default=Path("subclass_cluster_size_summary.tsv"))
    parser.add_argument("--out-detail", type=Path, default=Path("subclass_cluster_size_random_detail.tsv"))
    parser.add_argument("--out-plot", type=Path, default=Path("subclass_cluster_size_comparison.svg"))
...
def main() -> None:
    ...
    write_tsv(args.out_summary, summary); write_tsv(args.out_detail, detail); draw_svg(summary, args.out_plot)
