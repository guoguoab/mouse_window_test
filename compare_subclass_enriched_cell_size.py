#!/usr/bin/env python3
"""Compare true and random enriched-cell counts for each subclass.

Each row in a ``*_merged_regions_table_cell_id.txt`` file represents one
cluster.  Its enriched subclass/class cell count is compared with the mean
value from each random replicate (normally sample_1 ... sample_500).  Random
files are parsed in parallel because that is the expensive input step.
"""
...
VALUE_COLUMNS = ("enrich_subclass_cell_ids_num", "enrich_class_cell_ids_num")
...
def read_cluster_file(path: Path) -> tuple[str, dict[str, list[float]]]:
    """Return ``(sample, subclass -> enriched-cell counts)`` for one table."""
    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        ...
        value_column = next(
            (column for column in VALUE_COLUMNS if column in reader.fieldnames), None
        )
        if value_column is None:
            expected = "/".join(VALUE_COLUMNS)
            raise ValueError(f"文件缺少 {expected} 列: {path}")
        for row in reader:
            subclass = clean_subclass(row.get(subclass_column, "") or "")
            raw_count = (row.get(value_column) or "").strip()
            if not subclass or not raw_count:
                continue
            try:
...
def load_true(directory: Path) -> dict[str, list[float]]:
    ...
    for path in find_cluster_files(directory):
        ...
        for subclass, counts in by_subclass.items():
            combined[subclass].extend(counts)
    ...
    with ProcessPoolExecutor(max_workers=max(1, workers)) as executor:
        for sample, by_subclass in executor.map(read_cluster_file, files, chunksize=8):
            if not sample:
                continue
            for subclass, counts in by_subclass.items():
                by_sample[sample][subclass].extend(counts)
...
def compute_results(
    ...
    for subclass in sorted(true):
        ...
        for sample in samples:
            counts = random[sample].get(subclass, [])
            if not counts:
                continue
            ...
            detail.append({
                "subclass": subclass,
                "sample": sample,
                "random_cluster_n": len(counts),
                "random_mean_enriched_cell_num": sample_mean,
        ...
        summary.append({
            ...
            "random_sample_n": len(replicate_means),
            "random_mean_enriched_cell_num": random_mean,
            "random_median_enriched_cell_num": median(replicate_means),
            "random_q1": quantile(replicate_means, 0.25),
            "random_q3": quantile(replicate_means, 0.75),
            "random_min": min(replicate_means),
            "random_max": max(replicate_means),
...
def draw_svg(rows: list[dict[str, object]], path: Path) -> None:
    ...
    rows = sorted(rows, key=lambda row: float(row["true_mean_enriched_cell_num"]), reverse=True)
    ...
    maximum = max(
        max(float(row["random_max"]), float(row["true_mean_enriched_cell_num"]))
        for row in rows
    ) * 1.12 or 1
    ...
    out = [
        ...
        f'<text x="{width/2}" y="35" text-anchor="middle" font-size="24" font-family="sans-serif" font-weight="bold">True vs random mean enriched cell number by subclass</text>',
    ]
    for tick in range(6):
    ...
    for index, row in enumerate(rows):
        ...
        low, q1, med, q3, high = (
            float(row[key])
            for key in (
                "random_min", "random_q1", "random_median_enriched_cell_num",
        ...
        true_y = y(row["true_mean_enriched_cell_num"])
        out += [
    ...
    out += [
        f'<text transform="translate(20,{top+chart_h/2}) rotate(-90)" text-anchor="middle" font-size="15" font-family="sans-serif">Mean enriched cell number per cluster</text>',
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="按 subclass 比较真实与随机 cluster 的 enriched cell num，并绘制 SVG"
    )
    ...
    parser.add_argument(
        ...
        default=Path("subclass_enriched_cell_size_summary.tsv"),
    )
    parser.add_argument(
        "--out-detail", type=Path,
        default=Path("subclass_enriched_cell_size_random_detail.tsv"),
    )
    parser.add_argument(
        "--out-plot", type=Path,
        default=Path("subclass_enriched_cell_size_comparison.svg"),
    )
