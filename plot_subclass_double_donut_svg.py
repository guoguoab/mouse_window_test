...
def read_subclass_total(path: Path) -> Dict[str, float]:
    totals: Dict[str, float] = {}
    with path.open("r", encoding="utf-8") as f:
        reader = csv.DictReader(f, delimiter=';')
        for row in reader:
            name = (row.get("subclass") or "").strip()
    ...
    with path.open("r", encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            try:
                ...
                partner_n = 0.0
            rows.append({"sample_name": sample_name, "a_class": a_class, "enrich": enrich, "partner_n": partner_n})
    return rows
...
def main() -> None:
    ...
    totals_by_code = read_subclass_total(args.subclass_total)
    class_to_subcode = read_class_subclass_code_map(args.data_dir)
    rows = read_partner_rows(args.summary)
    ...
    for r in rows:
        sname, a_class, enrich, has_partner = r["sample_name"], r["a_class"], r["enrich"], r["partner_n"] > 0
        if sname == "true_data":
            true_clustered[a_class] += enrich
            (true_partner if has_partner else true_cluster_only)[a_class] += enrich
        elif sname.startswith("sample_"):
            sample_clustered[a_class][sname] += enrich
            (sample_partner if has_partner else sample_cluster_only)[a_class][sname] += enrich
    ...
    for a_class in a_classes:
        ...
        total = totals_by_code.get(sub_code, 0.0)
        ...
        parts = [
            ...
            '<rect x="390" y="140" width="16" height="16" fill="#8A8A8A"/><text x="412" y="153" font-size="14">cluster-only</text>',
            "</svg>",
        ]
