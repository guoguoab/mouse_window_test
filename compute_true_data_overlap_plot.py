...
def collect_overlap_values(total_rows: Iterable[Dict[str, object]], first: str, second: str) -> List[float]:
    values: List[float] = []
    for row in total_rows:
        a_type = str(row.get("a_cell_Neuron_type", ""))
        b_type = str(row.get("b_cell_Neuron_type", ""))

        if a_type == first and b_type == second:
            values.append(float(row["overlap_a_in_b"]))
        elif a_type == second and b_type == first:
            values.append(float(row["overlap_b_in_a"]))
    return values


def compute_distribution(values: List[float], bins: List[str]) -> Dict[str, float]:
    ...
    total = len(values)
    return {b: counts[b] * 100.0 / total for b in bins}
...
def main() -> None:
    ...
    for pair_name, (first, second) in pair_defs.items():
        values = collect_overlap_values(total_table, first, second)
        dist = compute_distribution(values, bins)
            ...
            out_pair_rows.append(
                {
                    ...
                    "overlap_bin": b,
                    "percent": dist[b],
                    "pair_count_used": total_n,
                }
            )

    out_pair = args.output_dir / args.pair_percent_table
    with out_pair.open("w", encoding="utf-8", newline="") as fh:
        writer = csv.DictWriter(fh, fieldnames=["pair_type", "overlap_bin", "percent", "pair_count_used"])
        writer.writeheader()
